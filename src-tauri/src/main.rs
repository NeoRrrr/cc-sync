...
fn resolve_project_root(config: &Value, config_path: &Path) -> PathBuf {
    sync_engine::project_root(config, config_path)
}
...
fn load_config_json(config_path: Option<String>) -> Result<(Value, PathBuf, PathBuf), String> {
    ...
    let value = sync_engine::load_config(&config_path)?;
...
fn runtime_config_context(
    ...
    if let Some(config) = config_override {
        let resolved_config_path = config_path
            .map(PathBuf::from)
            .unwrap_or_else(default_config_path);
        let config = sync_engine::normalize_config(
            config,
...
fn run_sync(
    ...
    let (runtime_config, resolved_config_path, _, _) =
        runtime_config_context(config_path, config_override)?;
    sync_engine::run_sync(&runtime_config, &resolved_config_path, &scope, dry_run)
}

#[tauri::command]
fn load_config_data(_app: tauri::AppHandle, config_path: Option<String>) -> Result<Value, String> {
    load_config_json(config_path).map(|(config, _, _)| config)
}
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            sync_engine::cleanup_legacy_python_runtime(&app_dir());
            setup_tray(app)?;
            Ok(())
        })
